...
Turning back to our proposed essay, the starting point was to configure ChatGPT so that it could begin to help us with this work. [[obsidian/Digital Garden/Goal-Directed Context Management#AI artefacts\|Goal-Directed Context Management#AI artefacts]] suggests we should have an *AI startup pack* to configure the AI for its initial role at the outset of a project. To obtain that, I began with a [[obsidian/6 - Main Notes/PHAIN/PHAIN bootstrap prompt\|bootstrap prompt]]. This gave the AI an explanation of the work we were about to do together, and tasked it to help me create an improved version of the prompt. 
...
With the startup pack now installed, I paused for thought. The question was: what outputs are required from the startup process? They might include:
...
Typically, all the above would be packaged in a *Project Brief*. An *Initiation Stage Plan* would also be needed. We would then be in a position to prepare the *Initiation Stage AI Pack*. This would set out how to configure the AI for that stage; i.e. to define the context AI would need in order to support the initiation stage most effectively. 
